            ...
            case Prim(op, expr_args):
                match op:
                    ...
                    case "eq" | "gt" | "gte" | "lt" | "lte":
                        assert tc_expr(expr_args[0], env) == tc_expr(expr_args[1], env)
                        return bool
...
def compress_parameters(prog: Program) -> Program:
    ...
    def cp_stmt(stmt: Stmt) -> Stmt:
        ...
        match stmt:
            ...
            case FunctionDef(name, params, body_stmts, return_type):
                func_def = FunctionDef(name, params, body_stmts, return_type)  # Prevents IDE type error
    ...
    def ea_stmt(stmt: Stmt) -> List[Stmt]:
        match stmt:
            ...
            case Assign(var, Prim("tuple", args)):
                ...
                all_stmts = [
                    Assign(new_freeptr_name, Prim("add", [Var("free_ptr"), Constant(bytes_needed)])),
                    Assign(less_than_var_name, Prim("lt", [Var(new_freeptr_name), Var("fromspace_end")])),
                ...
                all_stmts.append(
                    Assign(var, Prim("allocate", [Constant(bytes_needed), Constant(tag)]))
                )

                # Set contents #
                for i, arg in enumerate(args):
                    all_stmts.append(Assign("_", Prim("tuple_set", [Var(var), Constant(i), arg])))
...
def explicate_control(prog: Program) -> cfun.CProgram:
    ...
    def ec_stmt(s: Stmt, continuation: List[cfun.Stmt]) -> List[cfun.Stmt]:
        ...
        match s:
            ...
            case If(condition, then_stmts, else_stmts):
                ...
                then_label = create_block(ec_stmts(then_stmts, [cfun.Goto(continuation_label)]))

                else_label = create_block(ec_stmts(else_stmts, [cfun.Goto(continuation_label)]))
            ...
            case While(Begin(condition_stmts, condition_expr), body_stmts):
                ...
                body_label = create_block(ec_stmts(body_stmts, [cfun.Goto(test_label)]))

                new_continuation_stmts = [cfun.If(ec_expr(condition_expr),
                                                  cfun.Goto(body_label),
                                                  cfun.Goto(continuation_label))]

                new_condition_stmts = ec_stmts(condition_stmts, new_continuation_stmts)
    ...
    new_body = ec_stmts(prog.stmts, continuation=[cfun.Return(cfun.Constant(0))])
...
def select_instructions(prog: cfun.CProgram) -> X86ProgramDefs:
                ...
                match expr:
                    case cfun.Call(function_var, args):
                        ...
                        for arg in args:
                            ...
                            arg_instrs.extend([x86.NamedInstr("movq", [si_atm(arg), arg_register])])
                        ...
                        match op:
                            case "add" | "sub" | "mult" | "or" | "and":
                                return [x86.NamedInstr("movq", [si_atm(expr_args[0]), x86.Reg("rax")]),
                                        x86.NamedInstr(op_instructions[op], [si_atm(expr_args[1]), x86.Reg("rax")]),
                                        x86.NamedInstr("movq", [x86.Reg("rax"), x86.Var(var)])]
                            ...
                            case "eq" | "gt" | "gte" | "lt" | "lte":
                                return [x86.NamedInstr("cmpq", [si_atm(expr_args[1]), si_atm(expr_args[0])]),
                                        x86.Set(cmp_codes[op], x86.ByteReg("al")),
                            ...
                            case "allocate":
                                ...
                                return [x86.NamedInstr("movq", [x86.GlobalVal("free_ptr"), x86.Var(var)]),
                                        ...
                                        x86.NamedInstr("movq", [x86.Var(var), x86.Reg("r11")]),
                            ...
                            case "collect":
                                ...
                                return [x86.NamedInstr("movq", [x86.Reg("r15"), x86.Reg("rdi")]),
                                        x86.NamedInstr("movq", [x86.Immediate(bytes_to_collect), x86.Reg("rsi")]),
                                        x86.Callq("collect")]
    ...
    def si_def(func_def: cfun.CFunctionDef) -> X86FunctionDef:
        ...
        for arg_name in func_def.args:
            arg_register = x86.Reg(available_arg_registers.pop(0))
            arg_instrs.extend([x86.NamedInstr("movq", [arg_register, x86.Var(arg_name)])])
...
def allocate_registers(prog: X86ProgramDefs) -> X86ProgramDefs:
    ...
    for function_def in prog.defs:
        ...
        result_mini_prog: x86.X86Program = _allocate_registers(name=function_def.label, prog=mini_prog)
...
def _allocate_registers(name: str, prog: x86.X86Program) -> x86.X86Program:
    ...
    homes: Dict[x86.Var, x86.Arg] = {}  # variable homes; arg can be a register or a deref
        ...
        match instr:
            case x86.NamedInstr(i, [e1, e2]):
                match i:
                    ...
                    case _:
                        raise Exception("allocate_registers, writes_of, NamedInstr", i)
    ...
    def ul_block(label: str):
        ...
        block_live_after_sets: List[set] = []  # Start with empty list of live-after sets
    ...
    def color_graph(local_vars: Set[x86.Var],
        ...
        while len(vars_to_color) > 0:
            ...
            var_to_color = max(non_colored_sat_sets, key=lambda var: len(saturation_sets[var]))
            ...
            location_to_use = next(i for i in possible_locations if i not in saturation_sets[var_to_color])
    ...
    homes = color_graph(all_vars, interference_graph, regular_locations, tuple_locations)
...
def patch_instructions(prog: X86ProgramDefs) -> X86ProgramDefs:
    ...
    new_defs = []
    for function_def in prog.defs:
        mini_prog: x86.X86Program = x86.X86Program(blocks=function_def.blocks, stack_space=(0, 0))
